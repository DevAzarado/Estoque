...
namespace WebEstoque.Controllers
{
    public class ItemPedidoController : Controller
    {
        ...
        {
            var result = _context.ItensPedidos.Any(x => x.IdPedido == ped && x.IdProduto == prod);
            return result;
        }

        [HttpPost]
        public async Task<IActionResult> Cadastrar([FromForm] ItemPedido itemPedido)
        {
            if (ModelState.IsValid)
            {
                if (itemPedido.IdPedido > 0)
                {
                    ...
                    itemPedido.ValorUnitario = (double)produto.Preco;
